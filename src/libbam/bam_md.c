...
#define prob_to_sangerq(p) (p <= 0.0 ? 126+1 : ((int)(-10.0 * log10(p))+33))
...
/* this is a lofreq version of bam_prob_realn_core() which keep
 * baq as separate values instead of capping/merging it with base
...
int bam_prob_realn_lofreq(bam1_t *b, const char *ref, int extended, int redo)
{
	...
	uint8_t *qual = bam1_qual(b);
    /* Note that tags starting with X, Y and Z or tags containing
     * lowercase letters in either position are reserved for local use
     * and will not be formally deﬁned in any future version of this
     * speciﬁcation. */
    const char *BAQ_TAG = "lb";  /* private tag for actual baq values: "l"ofreq "b"ase-alignment: also defined in defaults.h */
    ...
    if (baq_full) {
         /* present and no redo: do nothing */
         return -3;
    }
...
int bam_prob_realn_core(bam1_t *b, const char *ref, int flag)
{
	...
	{ // glocal
		uint8_t *s, *r, *q, *seq = bam1_seq(b), *bq;
		int *state;
        int bw; /* grace's version redefined bw here without initializtion */
        ...
        // FIXME set pd and bw to zero if not in AQ mode
		kpa_glocal(r, xe-xb, s, c->l_qseq, qual, &conf, state, q, pd, &bw);
        ...
        {/***************************************************************
          ...
          ***************************************************************/
          ...
          iaq = calloc(c->l_qseq + 1, 1);/* insertion AQ */
          daq = calloc(c->l_qseq + 1, 1);/* deletion AQ */
		...
		free(bq); free(s); free(r); free(q); free(state);
        if (iaq && daq) {
                /* FIXME define somewhere and use local tag, i.e. lower case  */
