...
def cmdline_parser():
    ...
    parser.add_option("", "--strandbias-phred-auto",
                      dest="strandbias_phred_auto", 
                      action="store_true",
                      help="Optional: Automatically Bonferroni corrected strand bias phred value has to be 0.05")
...
def main():
    ...
    # has to be first filter
    if opts.strandbias_phred_auto:
        assert opts.max_strandbias_phred == None, (
            "Can't filter strand bias twice")
        bonffac = len(snps)
        tests.append((
            lambda s: float(s.info['strandbias-pvalue-uncorr'])*bonffac > 0.05,
            "automatically bonferroni corrected strandbias phred value"
            ))
        
    if opts.max_strandbias_phred != None:
        tests.append((
            lambda s: float(s.info['strandbias-pvalue-uncorr-phred']) <= opts.max_strandbias_phred,
            "minimum strandbias phred"
            ))
