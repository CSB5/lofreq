...
You will have to set the PICARDDIR environment variable to point to
your picard installation

  Options:
    -f | --fastq1   : Input fastq[.gz] file
    -g | --fastq2   : Fastq[.gz], second in pair (optional)
    -h | --help     : Display this help
    -i | --illumina : Phred qualities are ASCII64, ie. Illumina 1.3-1.7 (check with FastQC)
    -r | --ref      : Reference fasta file
    -s | --sanger   : Phred qualities are Sanger, ie. Illumina 1.8+
...
if [ $sanger -eq 1 ] && [ $illumina -eq 1 ]; then
    echo "ERROR: quality encoding can't be sanger *and* illumina. choose one" 1>&2
    exit 1
elif  [ $sanger -eq 0 ] && [ $illumina -eq 0 ]; then
    echo "ERROR: quality encoding must be one of sanger or illumina" 1>&2
    exit 1
elif [ $illumina -eq 1 ]; then
...
bwa $args | \
    grep '\(^@\|XT:A:U\)' | \
    samtools view -b -S $samtools_flag - | \
    java -jar $picard_readgroup_jar \
        INPUT=/dev/stdin OUTPUT=/dev/stdout \
        RGLB="library-dummy" RGPL="illumina" RGPU="platform-unit-dummy" RGSM="sample-name-dummy" | \
