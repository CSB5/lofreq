    ...
    for line in process.stdout:
        ...
        for this_snp in snp_candidates:
            bq = [(b, q) for (b, q) in zip(pcol.read_bases, pcol.base_quals)
                  if b == this_snp.wildtype or q >= opts.noncons_filter_qual]
            cov = len(bq)

            ref_count = len([(b, q) for (b, q) in bq if b == this_snp.wildtype])
            alt_count = len([(b, q) for (b, q) in bq if b == this_snp.variant])
            ...
            if USE_SCIPY:
                ...
                # FIXME
                raise ValueError, ("Haven't coded up cdf suuport yet. Use scipy")
